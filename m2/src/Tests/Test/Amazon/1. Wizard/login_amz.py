...
def amazon_log_in(a, b, c, browser):
    ...
    button = wait_presence_of_element("#signInSubmit", browser)
    try:
        ...
        edit.click()
        button = wait_presence_of_element("#addButton", browser)
        ...
        button = wait_presence_of_element("#addKeyButton", browser)
        ...
        checkbox.click()
        button = wait_presence_of_element("#auth-signin-button", browser)
        account = browser.find_element(By.XPATH, "//div [text() = 'M2EPRO_NA_TEST_1']")
        account.click()
        US = browser.find_element(By.XPATH, "//div [text() = 'United States']")
        US.click()
        button = wait_presence_of_element(".picker-switch-accounts-button", browser)
    except NoSuchElementException as err:
        pass
    check = browser.find_element(By.CLASS_NAME, "checkbox")
    check.click()
    button = wait_presence_of_element(".primary", browser)
    button = wait_presence_of_element(".primary", browser)
